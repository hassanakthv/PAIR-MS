...
fitPeak <- function(x, y, startPars = c(mu = 70.065, I = 5000, sigma = 3e-04), maxAttempts = 1000, sigma = 0) {
    fctStr <- function(x, mu, I, sigma) {
        I/sigma * exp(-1/2 * (x - mu)^2/sigma^2)
    }  # 'I/sigma*exp(-1/2*(x-mu)^2/sigma^2)'
    fitFct <- as.formula("y ~ fctStr(x, mu, I, sigma)")
    if (sigma > 0) {
        fctStr <- function(x, mu, I) {
            I/sigma * exp(-1/2 * (x - mu)^2/sigma^2)
        }
