...
aa_data %>%
  filter(ion %in% ion_plot) %>% 
  ggplot(aes(x=log(I0*gamma), y=gamma, group=file, color=group)) + 
...
aa_data %>%
  filter(ion %in% ion_plot) %>% 
  ggplot(aes(x=ltic, y=gamma, group=file, color=group)) + 
    geom_point(size=1, alpha=0.4) + #geom_smooth() +
    theme_bw() + 
    facet_grid(.~ion) + 
    theme(legend.position = "",axis.text.x = element_text(angle = 90, hjust = 1))+ 
...
aa_data %>% 
  filter(ion %in% ion_plot) %>% 
  ggplot(aes(x=log(I0*gamma), y=gamma, color=group, group=file)) + 
    geom_point(size=1, alpha=0.4) + #geom_smooth() +
    theme_bw() + 
    facet_grid(ion~.) + 
    theme(legend.position = "",axis.text.x = element_text(angle = 90, hjust = 1))+ 
...
aa_data %>% 
  ...
  ggplot(aes(x=tic, y=mean,  color=group)) +
    geom_point(size=3) + geom_smooth(method = "lm", se=F)+ geom_errorbar(aes(ymin=mean-3*se, ymax=mean+3*se),size=2)  +
    theme_bw() + facet_grid(.~ion, scales='free') + theme(axis.text.x=element_text(angle = 90, hjust = 1)) +ylab(bquote('Mean '^2*'H'/''^1*'H')) + xlab(expression(paste("TIC, ",Log[10],sep = ""))) -> g_tic
