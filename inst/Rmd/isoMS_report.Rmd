...
aa_data %>% 
  ...
  ggplot(aes(x=tic, y=mean,  color=group)) +
    ...
    theme_bw() + facet_grid(.~ion, scales='free') + ylab(bquote('Mean '^13*'C'/''^12*'C')) + xlab(expression(paste("TIC, ",Log[10],sep = ""))) -> g_tic

#grid.arrange(g_rt, g_logI, g_bp, g_density, ncol=1)

print(ggplotly(g_rt))
print(ggplotly(g_logI))
print(ggplotly(g_bp))
print(ggplotly(g_density))
print(ggplotly(g_tic))
```
...
aa_data %>% 
  ...
  ggplot(aes(x=tic, y=mean,  color=group)) +
    geom_point(size=3) + geom_smooth(method = "lm", se=F)+ geom_errorbar(aes(ymin=mean-3*se, ymax=mean+3*se),size=2)  +
    theme_bw() + facet_grid(.~ion, scales='free') + ylab(bquote('Mean '^15*'N'/''^14*'N')) + xlab(expression(paste("TIC, ",Log[10],sep = ""))) -> g_tic
...
aa_data %>% 
  ...
  ggplot(aes(x=tic, y=mean,  color=group)) +
    geom_point(size=3) + geom_smooth(method = "lm", se=F)+ geom_errorbar(aes(ymin=mean-3*se, ymax=mean+3*se),size=2)  +
    theme_bw() + facet_grid(.~ion, scales='free') + ylab(bquote('Mean '^2*'H'/''^1*'H')) + xlab(expression(paste("TIC, ",Log[10],sep = ""))) -> g_tic
...
aa_data %>% 
  ...
  aa_data %>% 
    ...
    ggplot(aes(x=tic, y=mean,  color=group)) +
      geom_point(size=3) + geom_smooth(method = "lm", se=F)+ geom_errorbar(aes(ymin=mean-3*se, ymax=mean+3*se),size=2)  +
      theme_bw() + facet_grid(.~ion, scales='free') + ylab(bquote('Mean '^18*'O'/''^16*'O')) + xlab(expression(paste("TIC, ",Log[10],sep = "")))-> g_tic
  #grid.arrange(g_rt, g_logI, g_bp, g_density, ncol=1)
