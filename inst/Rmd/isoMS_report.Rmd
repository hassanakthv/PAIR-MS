...
aa_data %>%
  filter(ion %in% ion_plot) %>% 
  ggplot(aes(x=log10(tic), y=100*gamma, group=file, color=group)) + 
...
aa_data %>% 
  filter(ion %in% ion_plot) %>% 
  ggplot(aes(x=log10(tic), y=1e6*gamma, color=group, group=file)) + 
    geom_point(size=1, alpha=0.4) + #geom_smooth() +
    theme_bw() + 
    facet_grid(.~ion) + 
    theme(legend.position = "",axis.text.x = element_text(angle = 90, hjust = 1))+ 
    xlab(expression(paste("TIC, ",Log[10],sep = ""))) + scale_x_log10() +
...
aa_data %>% 
  ...
  aa_data %>% 
    filter(ion %in% ion_plot) %>% 
    ggplot(aes(x=tic, y=100*gamma, color=group, group=file)) + 
      geom_point(size=1, alpha=0.2) + #geom_smooth() +
      theme_bw() + 
      facet_grid(ion~.) + 
      theme(legend.position = "",axis.text.x = element_text(angle = 90, hjust = 1))+ 
